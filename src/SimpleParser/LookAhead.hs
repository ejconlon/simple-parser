...
lookAheadMatch (MatchBlock sel dc mcs) = lookAheadParser sel >>= go mcs
 where
  go [] _ = dc
...
consumeMatch (MatchBlock sel dc mcs) = sel >>= go mcs
 where
  go [] _ = dc
  go ((MatchCase mcl mcg mch) : mcs') val =
    if mcg val
...
lookAheadTest (MatchBlock sel _ mcs) = go1
 where
  go1 s = do
...
lookAheadSimple sel dc pairs = lookAheadMatch (MatchBlock sel dc mcs)
 where
