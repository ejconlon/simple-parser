...
peekToken :: (Stream s, Monad m) => ParserT r s e m (Maybe (Token s))
peekToken = gets (fmap fst . streamTake1)

-- | Return the next token, if any, and consume it.
popToken :: (Stream s, Monad m) => ParserT r s e m (Maybe (Token s))
...
peekChunk :: (Stream s, Monad m) => Int -> ParserT r s e m (Maybe (Chunk s))
peekChunk n = gets (fmap fst . streamTakeN n)

-- | Return the next chunk of the given size, if any, and consume it.
-- May return a smaller chunk at end of stream, but never returns an empty chunk.
popChunk :: (Stream s, Monad m) => Int -> ParserT r s e m (Maybe (Chunk s))
...
satisfyToken :: (Stream s, Monad m) => (Token s -> Bool) -> ParserT r s e m (Token s)
satisfyToken p = do
  m <- popToken
  case m of
    Just c | p c -> pure c
...
foldTokensWhile :: (Stream s, Monad m) => (Token s -> x -> (Bool, x)) -> x -> ParserT r s e m x
...
takeTokensWhile :: (Stream s, Monad m) => (Token s -> Bool) -> ParserT r s e m (Chunk s)
takeTokensWhile = state . streamTakeWhile

-- | Take tokens into a chunk while they satisfy the given predicate.
-- Only succeeds if 1 or more tokens are taken, so it never returns an empty chunk.
takeTokensWhile1 :: (Stream s, Monad m) => (Token s -> Bool) -> ParserT r s e m (Chunk s)
...
dropTokensWhile :: (Stream s, Monad m) => (Token s -> Bool) -> ParserT r s e m Int
dropTokensWhile = state . streamDropWhile

-- | Drop tokens and return chunk size while they satisfy the given predicate.
-- Only succeeds if 1 or more tokens are dropped.
dropTokensWhile1 :: (Stream s, Monad m) => (Token s -> Bool) -> ParserT r s e m Int
...
matchToken :: (Stream s, Monad m, Eq (Token s)) => Token s -> ParserT r s e m (Token s)
matchToken t = do
  mu <- popToken
  case mu of
    Just u | t == u -> pure u
...
matchChunk :: (Stream s, Monad m, Eq (Chunk s)) => Chunk s -> ParserT r s e m (Chunk s)
matchChunk k = do
